# Данный скрипт деплоит на scratch org локальный исходный код (если есть) и устанавливает
...
# $ ./scripts/SetupScratchOrg/deployMetadataAndInstallPackages.sh

# Сам файл при этом должен быть исполняемым, для этого нужно один раз выполнить команду: 
# chmod +x ./scripts/SetupScratchOrg/deployMetadataAndInstallPackages.sh
...
# sed -i.bak 's/\r$//' ./scripts/SetupScratchOrg/deployMetadataAndInstallPackages.sh
